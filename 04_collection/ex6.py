...
d.update(d2)        # key가 있으면 update, 없으면 insert -> upsert
print(d)
...
result = {i: i ** 2 for i in range(1, 11) if i % 2 == 1}
print(result)
...
# 점수 90 이상만 필터링하기
scores = {"국어": 90, "영어": 85, "수학": 96, "과학": 80}
...
# Counter: 요소의 빈도수를 세어주는 딕셔너리 서브클래스
...
result = {k: "합격" if v >= 60 else "불합격" for k, v in scores.items()}
print(result)                                    # ✅ {'국어': '합격', '수학': '합격', '사회': '합격'}


# 3 과목 리스트와 점수 리스트로 딕셔너리 만들기
subjects = ["국어", "영어", "수학"]
grades = [90, 80, 100]
...
# 4 기존 재고에 입고 내역을 합치기 (이미 있는 상품은 합산, 새 상품은 추가)
...
# Dictionary Comprehension no.2
result = {k: stock.get(k, 0) + incoming.get(k, 0) for k in set(stock) | set(incoming)}
